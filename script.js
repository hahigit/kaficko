...
function renderDrinks(drinksApiResponse) {
  ...
  list.forEach((d, idx) => {
    ...
    header.className = "drink__header";
    header.innerHTML = `
      <div class="drink__name">${typeName}</div>
      <div class="drink__val" id="val-${idx}">0</div>
    `;

    const inp = document.createElement("input");
    inp.type = "range";
    inp.className = "drink__slider";
    inp.min = "0";
    inp.max = "10";
    inp.step = "1";
    inp.value = "0";
    inp.dataset.type = String(typeName);
    inp.dataset.id = `val-${idx}`; 
...
async function handleSubmit(e) {
  ...
  try {
    ...
    document.querySelectorAll("input[data-type]").forEach(inp => {
      inp.value = 0;
      document.getElementById(inp.dataset.id).textContent = "0";
    });
